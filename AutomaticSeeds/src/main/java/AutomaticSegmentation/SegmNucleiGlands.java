...
public class SegmNucleiGlands {
	...
	private void segmentationProtocol(ImagePlus initImp, boolean gpuOption) {
		...
		ImagePlus imp_segmented = automaticThreshold(initImp);
		 ...
		 ImageStack imgFilterSmall = BinaryImages.volumeOpening(imgFilled,
		 50);
		 ...
		 ImageStack imgGradient = Morphology.gradient(imgFilterSmall,
		 shape3D);
		 ...
		 MinimaAndMaxima3D.extendedMinima(imgGradient,
		 this.toleranceWatershed, conn);
		 ...
		 MinimaAndMaxima3D.imposeMinima(imgGradient, regionalMinima, conn);
		 // progressBar.show(0.5);
		 imgToShow = new ImagePlus("imposeMinima", imposedMinima);
		 imgToShow.show();
		 ...
		 try {
		 labeledMinima = BinaryImages.componentsLabeling(regionalMinima, conn,
		 BitD);
		 } catch (Exception e) {
		 ...
		 labeledMinima =
		 BinaryImages.componentsLabeling(regMinip.getImageStack(), conn,
		 ...
		 labeledMinima, conn, dams);
		 // progressBar.show(0.85);
		
		 imgToShow = new ImagePlus("Watershed", resultStack);
		 imgToShow.show();
		 ...
		 ImageStack imgFilterSize = LabelImages.volumeOpening(resultStack,
		 (int) Math.round(thresholdVolume));
		
		 /*** get colored labels and return image ***/
		 // create image with watershed result
		 ImagePlus imp_segmentedFinal = new ImagePlus("filtered size",
		 imgFilterSize);
		 ...
		 ImageConverter converterFinal = new
		 ImageConverter(imp_segmentedFinal);
		 converterFinal.convertToGray8();
		 // Color image
		 byte[][] colorMap =
		 CommonLabelMaps.fromLabel(CommonLabelMaps.GOLDEN_ANGLE.getLabel()).computeLut(255,
		 false);
		 //Border, color
...
//		}
		...
		for (int i = 1; i <= initImp.getStackSize(); i++) {
			...
			//System.out.println(processor.getAutoThreshold() + " " + processor.getMinThreshold() + " " + processor.getMaxThreshold());
			thresholds.add((int) processor.getMaxThreshold()); //Getting max instead of threshold, we similar results compared with Fiji's
		}
